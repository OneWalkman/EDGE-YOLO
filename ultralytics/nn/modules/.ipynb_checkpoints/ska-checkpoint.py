...
try:
    ...
    custom_fwd = lambda f=None, **kwargs: f if f else (lambda x: x)
    custom_bwd = lambda f=None, **kwargs: f if f else (lambda x: x)
...
class SKA(torch.nn.Module):
    def forward(self, x: torch.Tensor, w: torch.Tensor) -> torch.Tensor:
        assert x.is_cuda and w.is_cuda, f"SkaFn requires CUDA tensors. Got: x={x.device}, w={w.device}"
