...
class Attention(torch.nn.Module):
    ...
    def forward(self, x):
        ...
        Attention._call_id += 1
    
        # ---------- 重建相对位置索引 ----------
        if N != self.attention_bias_idxs.numel():
            ...
            idxs = torch.as_tensor(idxs, device=x.device, dtype=torch.long).view(N, N)
            self.register_buffer('attention_bias_idxs', idxs, persistent=False)
            self.attention_biases = torch.nn.Parameter(
        ...
        print(f"[ATTN {Attention._call_id:02d}]  "
              f"stage_hw={H}x{W}  q{q.shape} k{k.shape}  heads={self.num_heads} key_dim={self.key_dim}")
